...
function adaptResult(raw: Record<string, any>): AcceptanceResult {
...
function adaptRun(raw: Record<string, any>): AcceptanceRun {
  ...
  return {
    ...
    status,
    results: report.map(adaptResult),
  };
}
...
export const acceptance = {
  async start(): Promise<AcceptanceRun> {
    ...
    const raw = await apiFetch<Record<string, any>>("/acceptance/run", { method: "POST" });
  ...
  async status(runId: string): Promise<AcceptanceRun> {
    const raw = await apiFetch<Record<string, any>>(`/acceptance/status/${runId}`);
    return adaptRun(raw);
  },
};
